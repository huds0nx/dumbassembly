...
typedef mdToken mdMethodDef;            // MethodDef in this scope
...
typedef enum CorMethodAttr
{
    ...
    mdFinal                     =   0x0020,     // MethodDef may not be overridden.
    mdVirtual                   =   0x0040,     // MethodDef virtual.
    mdHideBySig                 =   0x0080,     // MethodDef hides by name+sig, else just by name.

    // vtable layout mask - Use this mask to retrieve vtable attributes.
    mdVtableLayoutMask          =   0x0100,
    mdReuseSlot                 =   0x0000,     // The default.
    mdNewSlot                   =   0x0100,     // MethodDef always gets a new slot in the vtable.
    // end vtable layout mask

    // method implementation attributes.
    mdCheckAccessOnOverride     =   0x0200,     // Overridability is the same as the visibility.
    mdAbstract                  =   0x0400,     // MethodDef does not provide an implementation.
    mdSpecialName               =   0x0800,     // MethodDef is special.  Name describes how.
    ...
    mdHasSecurity               =   0x4000,     // MethodDef has security associate with it.
    mdRequireSecObject          =   0x8000,     // MethodDef calls another method containing security code.
...
typedef enum CorMethodImpl
{
    ...
    miIL                =   0x0000,   // MethodDef impl is IL.
    miNative            =   0x0001,   // MethodDef impl is native.
    miOPTIL             =   0x0002,   // MethodDef impl is OPTIL
    miRuntime           =   0x0003,   // MethodDef impl is provided by the runtime.
    // end code impl mask

    // managed mask
    miManagedMask       =   0x0004,   // Flags specifying whether the code is managed or unmanaged.
    miUnmanaged         =   0x0004,   // MethodDef impl is unmanaged, otherwise managed.
    miManaged           =   0x0000,   // MethodDef impl is managed.
    // end managed mask
    ...
    miSynchronized      =   0x0020,   // MethodDef is single threaded through the body.
    miNoInlining        =   0x0008,   // MethodDef may not be inlined.
