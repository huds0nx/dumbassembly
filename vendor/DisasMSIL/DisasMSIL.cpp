    ...
    #define ADDN(n,nt)				{																			\
		...
		case NUMBER_TYPE_SMALL_BRANCH:																		\
			if (((BYTE) n) <= 127)																			\
				...
				sprintf_s(szNumber, 100, "0x%08X", (DWORD) (Base + 2) - (0 - (char) n));							\
			break;																							\
		case NUMBER_TYPE_BRANCH:																			\
			if (((DWORD) n) <= 0x7FFFFFFF)																	\
				sprintf_s(szNumber, 100, "0x%08X", (DWORD) (Base + 5) + n);									\
			else																							\
				sprintf_s(szNumber, 100, "0x%08X", (DWORD) (Base + 5) - (0 - (int) n));							\
			break;																							\
